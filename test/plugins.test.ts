...
import { awsyDefinitionPlugin, adaptConfig } from "../src/definitions/awsy/index.js";
...
// ─── awsy definition plugin ─────────────────────────────────

describe("awsy definition plugin", () => {
  test("canLoad matches yml and yaml extensions", () => {
    expect(awsyDefinitionPlugin.canLoad("awsy.yml")).toBe(true);
    expect(awsyDefinitionPlugin.canLoad("config.yaml")).toBe(true);
    expect(awsyDefinitionPlugin.canLoad("config.json")).toBe(false);
    expect(awsyDefinitionPlugin.canLoad("serverless.yml")).toBe(true);
  });
  ...
  test("formatName is awsy", () => {
    expect(awsyDefinitionPlugin.formatName).toBe("awsy");
  });
});
